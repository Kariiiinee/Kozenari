                ...
                <div className="bg-[#f6f8f6] p-5 border-b border-slate-100 flex justify-between items-center shrink-0">
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2 text-slate-500 text-xs font-bold uppercase tracking-wider mb-1">
                            ...
                            {new Date(scan.timestamp).toLocaleDateString(i18n.language, { month: 'short', day: 'numeric', year: 'numeric' })}
                        </div>
                        <div className="flex items-center gap-1.5 text-slate-400 text-[10px]">
                            <Clock className="w-3 h-3" />
                            {new Date(scan.timestamp).toLocaleTimeString(i18n.language, { hour: '2-digit', minute: '2-digit' })}
                        </div>
                    </div>
                ...
                {scan.vibe && (
                    <div className="bg-white p-6 pb-2 text-center shrink-0">
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#13ec13]/10 rounded-full">
                            <span className="text-2xl font-bold text-[#13ec13]">
                                {t(`scan.vibes.${scan.vibe.toLowerCase()}.emoji`, { defaultValue: scan.vibe })} {t(`scan.vibes.${scan.vibe.toLowerCase()}.label`, { defaultValue: scan.vibe })}
                            </span>
                        </div>
                    </div>
                )}
                ...
                <div className="p-6 pt-2 overflow-y-auto space-y-6">
                    ...
                    {scan.microActions && scan.microActions.length > 0 && (
                        <div className="space-y-3">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('modal.recommended_actions')}</h4>
